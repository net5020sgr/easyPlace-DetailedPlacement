...
void PlaceDB::outputDEF(string suffix,string init_def_path){
    std::ifstream in(init_def_path);
    if (!in.is_open()) {
        std::cerr << "Failed to open initial DEF file: " << init_def_path << "\n";
        return;
    }
    string outputFilePath;
    string benchmarkName;
    ...
    outputFilePath = outputFilePath + benchmarkName + "-" + suffix + ".def";
    std::ofstream out(outputFilePath);
    if (!out.is_open()) {
        std::cerr << "Failed to open output DEF file: " << outputFilePath << "\n";
        return;
    }
    string line;
    ...
    while (std::getline(in, line)) {
        ...
        if (line.find("END COMPONENTS") != string::npos && inComponent == true) {
            ...
            // Lookup
            auto module = getModuleFromName(nodeName);
